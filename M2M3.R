...
# saved into M2M3_1.csv, M2M3_2.csv, M2M3_3.csv, 
# M2M3_4.csv, which is called by 
# maketables.R to produce results in Table 5
# of the paper by Chen, Harel and Ravishanker.

######UConn physic cluster
run <- commandArgs(TRUE)
args <- as.numeric(run)
print(c("args", args))

# iter is the processor index #
iter = args +1
print(c("iter", iter))
...
write.csv(as.matrix.data.frame(para.tot1), paste0("M2M3_", iter, ".csv", sep = ""))
