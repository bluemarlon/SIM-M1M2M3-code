# Code for Simulating data from model M2 and 
# fitting model M2. Results from 100 replications 
# saved into M2M2_1.csv, M2M2_2.csv, M2M2_3.csv, 
# M2M2_4.csv, which is called by 
# maketables.R to produce results in Table 5
...
for(iter in 1:4){
  ...
  for(kkkk in 1:100){
    # start of data simulation from M2 model
    para.tot = data.frame()
    ...
    # end of data simulation from M2 model
    
    
    ctrl <- lmeControl(opt='optim');
    fm.repeat <<- lme(yjt ~ X1 +X2, random = ~0+ X2|PID ,control=ctrl,
  ...
  write.csv(as.matrix.data.frame(para.tot1), paste0("M2M2_", iter,".csv", sep = ""))
}
